...
The submitted resolver remains deterministic. An opt-in experimental `enhance` workflow now implements hybrid candidate retrieval, Jev identity decisions, consistent provisional clusters (including companies without identifiers), and evidence-preserving profile reconciliation. It operates on the same frozen record selection and leaves the submission untouched. This implementation is not a demonstrated production migration or a measured accuracy improvement.

Fuzzy names handle spelling and suffix differences; embeddings broaden candidate retrieval. Jev judges supplied identity, corroboration and ownership evidence. Matching names alone cannot establish identity. Sparse pairs can receive model assessments and appear as unreviewed candidates without becoming links. Admission requires independent corroboration: full registered/business addresses, entity website domains, or constrained distinctive-name/location combinations. Service-location combinations also require clear entity ownership. Common/shared values are flagged. Conflicting identifiers block admission, bridges cannot merge existing company IDs, and identifier-free groups require every cross-source pair to pass. Model scores of at least 0.98 and a 0.10 identity margin are initial experimental gates, not calibrated probabilities.

For profiles, Jev can group equivalent field representations, but the selected representative is an original source value. Original claims, address bundles and distinct roles remain visible. The configured LLM annotates unresolved conflicts with observation citations and an inference/insufficient-evidence outcome. Models cannot silently rewrite source claims.

Comparison reports cover exact-only, fuzzy-plus-Jev and fuzzy-plus-embedding-plus-Jev on identical records. Entity-grouped development/heldout worksheets include outside-candidate audits and separate identifier-masked controls; unknown labels never count as correct. The 99% observed semantic precision target remains unverified until human review. Benchmark recall is not population recall because the baseline selection favors identifier overlap. Enhancement embeddings, decisions, explanations and failed attempts have a separate usage ledger; historical zero per-record model cost above describes only the saved submission. [Operations and evaluation](docs/ENGINEERING.md#experimental-hybrid-enhancement).
